...
int main(int argc, char *argv[])
{
  ...
  if (miId == 0)
  {
    ...
    {
      int currentIndex = i * dimVectorLocal;
      MPI_Send(&vectorInicial[currentIndex], dimVectorLocal, MPI_DOUBLE, i, 33, MPI_COMM_WORLD);
    }
  }
  else
  {
    MPI_Status s;
    MPI_Recv(vectorLocal, dimVectorLocal, MPI_DOUBLE, 0, 33, MPI_COMM_WORLD, &s);
  }
